...
namespace WpfAppPLC
{
    public partial class MainWindow : Window
    {
        ...
        private const int MX_FIR_HOLD = 36;  // %MX36 1층 유지(목적/상태 판단용)
        private const int MX_SEC_HOLD = 37;  // %MX37 2층 유지(목적/상태 판단용)

        private const int MX_DOOR_CLOSE_MTR = 40;  // %MX40 문닫힘모터
        private const int MX_DOOR_OPEN_MTR = 41;  // %MX41 문열림모터

        // (옵션) 외부 호출 관련 상태 비트가 따로 있으면 모니터링에 추가
        ...
        // ============================================================
        // 2) XGCommLib 통신 객체 (연동 "잘 되는 방식" 그대로)
        //    - CommObjectFactory20 → GetMLDPCommObject20(endpoint)
        //    - AddDeviceInfo를 1번 구성하고 Read/WriteRandomDevice 사용
        // ============================================================

        private readonly CommObjectFactory20 _factory = new CommObjectFactory20();

        // 상태 읽기 전용 드라이버(목록 구성 후 ReadRandomDevice 반복)
        private CommObject20? _drvStatus;
        private readonly List<DeviceKey> _statusOrder = new(); // AddDeviceInfo 순서(=버퍼 인덱스)
        private int _statusBufLen;

        // 명령 쓰기 전용 드라이버(가능하면 별도 연결)
        private CommObject20? _drvCmd;
        private readonly List<DeviceKey> _cmdOrder = new();
        private int _cmdBufLen;
        private bool _cmdUsesOneShot = false; // 2번째 연결이 막히면 OneShot 방식 사용
        ...
        // CarTranslate.Y 기준
        private const double CAR_Y_1F = 0.0;        // 1층 위치
        private const double CAR_Y_2F = -380.0;     // 2층 위치(층 라인 간격 380px)

        // 문 열림 폭
        private const double DOOR_OPEN_X = 60.0;

        // 래더 타이머에 맞춘 “느낌” 시간(필요하면 조절)
        private static readonly TimeSpan TRAVEL_TIME = TimeSpan.FromSeconds(7); // 이동 7초
        private static readonly TimeSpan DOOR_TIME = TimeSpan.FromSeconds(3); // 문 3초
        ...
        private bool _doorOpening = false; // true=열기, false=닫기
        ...
        private void Connect(string endpoint)
        {
            ...
            BuildStatusDeviceList(); // AddDeviceInfo는 “1회만” 구성

            // (2) 명령 쓰기 드라이버 연결(가능하면 별도 연결)
            ...
            _ = Task.Run(async () =>
            {
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        if (st.HasValue)
                        {
                            var prev = _last;      // 이전 상태(애니메이션 트리거용)
                            _last = st.Value;      // 최신 상태
                    ...
                    await Task.Delay(100, token); // 0.1초 주기(원하시면 50/200으로 조절)
                }
            }, token);
        }
        ...
        private void UpdateUiFromStatus(PlcStatus prev, PlcStatus cur)
        {
            Dispatcher.Invoke(() =>
            {
                ...
                int floor = cur.FirFloor ? 1 : (cur.SecFloor ? 2 : 0);

                Text_CurrentFloor.Text = floor == 0 ? "-" : floor.ToString();
                Text_CarFloor.Text = floor == 0 ? "-" : $"{floor}F";
                ...
                if (cur.EvMoving && !prev.EvMoving)
                {
                    ...
                    else _carToY = _carFromY; // 판단 불가 시 제자리
                }
        ...
        private void SetConnectedUi()
        {
            Dispatcher.Invoke(() =>
            {
                ...
                Text_ConnState.Foreground = BrushFrom("#FF059669"); // Green-ish
            });
        }
        ...
        // ============================================================
        // 11) 로그 출력(화면 하단 TextBox_Log에 누적)
        // ============================================================
        ...
        private readonly struct DeviceKey : IEquatable<DeviceKey>
        {
            ...
            public readonly int Index;  // MX bit or MB byte offset
