...
### 주요 기능
- **실시간 데이터 동기화**: PLC의 메모리(%M)를 0.3초 주기로 스캔하여 WPF UI에 실시간 반영
...
| **데이터** | %MB100 | 현재층수 | 엘리베이터의 현재 위치 정보 (Byte) |

### 2. PLC 래더 로직 구조

- **시퀀스 제어**: 타이머(TON)를 사용하여 문 개폐 속도를 3초로 제어
- **데이터 처리**: `MOVE` 명령어를 사용하여 도착 완료 시 해당 층수 데이터를 `%MB100`에 저장
...
<div align="center">
  ...
  <p><i>PLC 시뮬레이터와 C# WPF 연동 엘리베이터 제어 시연 영상 (64s)</i></p>
</div>
